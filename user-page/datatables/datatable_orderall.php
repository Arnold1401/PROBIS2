...
$query = "SELECT * FROM hjual h, customer c, sales s WHERE (h.id_hjual LIKE '%".$search."%' OR h.status_order LIKE '%".$search."%' OR h.tanggal_order LIKE '%".$search."%' OR h.tanggal_orderselesai LIKE '%".$search."%' OR c.id_cust LIKE '%".$search."%' OR s.nama_sales LIKE '%".$search."%' OR h.kurir LIKE '%".$search."%') and c.id_cust=h.id_cust and h.id_cust='$idcust' and s.id_sales=c.id_sales and h.status_pembayaran='Lunas' OR h.status_pembayaran='Batal' GROUP BY h.id_hjual";
