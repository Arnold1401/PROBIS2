...
$getsaltId = $_POST['get_idsal'];

$sql = mysqli_query($connect, "SELECT email FROM sales where id_sales=$getsaltId"); // Query untuk menghitung seluruh data siswa
$sql_count = mysqli_num_rows($sql); // Hitung data yg ada pada query $sql

$query = "SELECT * FROM sales s WHERE (nomor_telepon LIKE '%".$search."%' OR nama_sales LIKE '%".$search."%' OR email LIKE '%".$search."%' OR no_ktp LIKE '%".$search."%') and id_sales=$getsaltId";
